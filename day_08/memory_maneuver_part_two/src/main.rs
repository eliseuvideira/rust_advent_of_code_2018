...
fn calc_node(node: &Node) -> u32 {
    let name = (node.id as u8 + b'A') as char;

    let node_value: u32 = match node.child_nodes.len() {
        0 => node
            .metadata_entries
            .iter()
        ...
        _ => node
            .metadata_entries
            .iter()
            .filter_map(|entry| node.child_nodes.get((entry - 1) as usize))
            .map(|node| calc_node(node))
            .sum(),
    };

    println!("{:2}--{:8}", name, node_value);
