...
fn main() {
    ...
    for (guard_id, minutes_sleep) in minutes_asleep.iter() {
        let (max_minute, total_minutes) = get_totals(minutes_sleep);

        if total_minutes > current_max.2 {
            current_max = (guard_id.clone(), max_minute, total_minutes);
        }
    }
...
fn get_totals(minutes_sleep: &HashMap<u32, u32>) -> (Minute, u32) {
    ...
    return (max_minute, total_minutes);
}
